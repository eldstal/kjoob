class App:
  ...
  def get_app_id(appclass):
    for (i, (m,s)) in App.known_apps.items():
      if (m == appclass or s == appclass): return i
    return None

  @staticmethod
  def register_app(masterclass, slaveclass):
    ...
    App.next_app_id += 1
    App.known_apps[App.next_app_id] = (masterclass, slaveclass)
    return App.next_app_id

  @staticmethod
  def get_slave_app(app_id):
    (m, s) = App.known_apps[app_id]
    return s
